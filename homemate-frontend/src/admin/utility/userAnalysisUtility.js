...
export const generateTimeRanges = (grouping, year, month, date) => {
    const now = new Date(); // internally UTC-based
    const ranges = [];
...
export const formatRangeLabel = (range, grouping) => {
    ...
    const fromStr = range.from; // Format: "YYYY-MM-DDTHH:mm:ss"

    if (grouping === 'year') {
        return fromStr.substring(0, 4); // Extract year: "YYYY"
    } else if (grouping === 'month') {
        return fromStr.substring(0, 7); // Extract year-month: "YYYY-MM"
    } else if (grouping === 'day') {
        return fromStr.substring(0, 10); // Extract date: "YYYY-MM-DD"
    }
    return range.from;
};
