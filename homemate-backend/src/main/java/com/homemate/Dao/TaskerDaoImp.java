...
public class TaskerDaoImp implements TaskerDao {
    ...
    public List<Tasker> findTaskersWithFilters(FindTaskerCriteriaDto filter, int page, int pageSize) {
        ...
        sql.append("SELECT taskerID, firstName, lastName, image, rating, availability, bio, addressCity, hourRate FROM Tasker WHERE 1=1");
        ...
        if (filter.getAvailability() != null && !filter.getAvailability().isEmpty()) {
            sql.append(" AND availability = ?"); params.add(filter.getAvailability());
        }
        ...
        if (filter.getSortBy() != null && !filter.getSortBy().isEmpty()) {
            ...
            sql.append(" DESC".equalsIgnoreCase(filter.getSortOrder()) ? " DESC" : " ASC");
        }
